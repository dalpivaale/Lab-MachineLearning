...
pred0 <- predict(logis0, pima.data, type="response")#abbiamo il nostro modello/classificatore, prediciamo con esso sempre il pima data e vediamo se predice bene, con type = response ti da la probabilità per ogni individuo
...
pred <- predict(logis, tst, type="response")# probabilità calcolate nel testset, prediciamo sul test data che non avevamo usato nella costruzione del modello
...
test_roc = roc(tst$diabetes ~ nb_prob[,2], plot = TRUE, print.auc = F,col="pink", add=T,print.thres="best")#devi beccarti la seconda colonna dell'oggetto sopra è la prob. di essere pos
